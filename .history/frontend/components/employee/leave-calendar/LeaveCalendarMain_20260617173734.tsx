    ...
    const fetchLeaves = async () => {
      ...
      try {
        ...
        if (selectedDate?.from) {
          queryParams += `&startDate=${format(selectedDate.from, "yyyy-MM-dd")}`;
        }
        if (selectedDate?.to) {
          queryParams += `&endDate=${format(selectedDate.to, "yyyy-MM-dd")}`;
        } else if (selectedDate?.from) {
          queryParams += `&endDate=${format(selectedDate.from, "yyyy-MM-dd")}`;
        }
        const res = await api.get("/leaves/approved${queryParams}");
        setLeaves(res.data);
      } catch (err) {
        console.error(err);

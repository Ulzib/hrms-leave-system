...
export const useReadRequestIds = () => {
  ...
  const markRead = (id: number) => {
    setReadIds((prevReadIds) => {
      const alreadyRead = prevReadIds.includes(id);
      if (alreadyRead) return prevReadIds;

      const updateIds = [...prevReadIds, id];
    ...
    <div>
      <Popover>
        <PopoverTrigger asChild>
          <Button className="border-[#E4E4E7] border-dashed py-2.5 px-3 gap-2 rounded-md bg-[#FFFFFF] dark:bg-gray-200 text-black">
        ...
        <PopoverContent className="w-56 p-1" align="start">
          <div className="flex flex-col">
            {STATUS_OPS.map((op) => (
              <label
                key={op.label}
                className="flex items-center justify-between gap-2 rounded-sm px-2 py-1.5 text-sm cursor-pointer hover:bg-muted"
              >
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={selectedStatuses.includes(op.value)}
                    onChange={() => toggleStatus(op.value)}
                    className="size-4 rounded border-gray-300 accent-black cursor-pointer"
                  />
                  {op.label}
                </div>
                <p className="text-muted-foreground">{counts[op.value] ?? 0}</p>
              </label>
            ))}
          </div>
        </PopoverContent>
